...
class SudokuConfig:
    ...
    def remove_choice( self, board, coord, n ):
        idx = self.index( coord )
        board[idx][0] -= board[idx][n]
        board[idx][n] = 0
       	if board[idx][0] == 1:
       		self.fix_point(self, board, coord, n)
       	elif board[idx][0] == 0:
 	       	raise Impossible
...
class SudokuBoard:
    def __init__( self, small_size = None, size = None ):
        ...
        self.board = [[self.config.size()]+[1]*self.config.size() for j in self.config.rSize for k in self.config.rSize]
